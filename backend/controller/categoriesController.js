...
const createCategory = async (req, res) => {
    try {
        ...
        if(categoryBody == null || categoryBody == undefined  || categoryBody == {})
            res.status(404).json({ error: "Invalid Parameters !", status:false });
        else if(categoryBody.category == null || categoryBody.category == "" || categoryBody.category.length <= 0)
            res.status(404).json({ error: "Please enter a category", status:false });
        else {
            const duplicateDataExists = await validateDuplicateCategory(categoryBody.category);
            if(duplicateDataExists)
                res.status(200).json({ "message": "This category already exists", status:false });
            else{
                const createResult = await Category.create(categoryBody);
                res.status(200).json({ "createResult": createResult, "status": true });
            }
        }
    }
    catch(error) {
...
const getAllCategories = async (req, res) => {
    try {
        ...
        if(fetchedData) 
            res.status(200).json({ categoriesList: fetchedData, status: true });
        else
            res.status(200).json({ "message" : "No Records Found !", status: false});
    }
    catch (error) {
...
const editCategory = async (req, res) => {
    try {
        ...
        if(updateBody == null || updateBody == undefined || updateBody == {})
            res.status(404).json({ error: "Insufficient paramters !", status: false });
        else {
...
const deleteCategory = async (req, res) => {
    try {
        const id = String(req.params.id) || ""
        if(id == null || id == undefined || id == "" || id.length <= 0)
            res.status(404).json({ error: "Id Not Found", status: false });
        else if (!HelperService.isValidObjectId(id))
            res.status(404).json({ error: "Invalid id", status: false });
        else {
